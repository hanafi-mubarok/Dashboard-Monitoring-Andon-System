...
export default function PengerjaanLantai1Page() {
  ...
  const [allData, setAllData] = useState<ProductPercentageLantai1[]>([]);
  ...
  useEffect(() => {
    const fetchProjectOptions = async () => {
      try {
        const res = await fetch('/api/product/project-options-lantai1');
  ...
  useEffect(() => {
    ...
    const fetchFilteredData = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/product/percentage-lantai1?${params.toString()}`);
...
// Set default trainset only when project changes (do not depend on selectedTrainset to avoid reset on user selection)
  useEffect(() => {
    ...
    const fetchLatest = async () => {
      ...
      try {
        const res = await fetch(`/api/product/latest-trainset?project=${encodeURIComponent(selectedProject)}&line=Lantai%201&workshop=Candisewu`);
        if (res.ok) {
          const json = await res.json();
          if (json?.trainset) {
  ...
  useEffect(() => {
    ...
    // Keep selectedTrainset only if it's still valid for current project
            ...
            <div className="space-y-6">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
                <div className="flex items-center gap-3">
                  <label htmlFor="project-filter-lantai1" className="text-sm font-medium text-gray-300">Project</label>
                  <select
                    id="project-filter-lantai1"
              ...
              {data.length > 0 ? (
                <>
                  ...
                  {effectiveTrainset && <ProductSummaryListLantai1 trainset={effectiveTrainset} project={selectedProject} />}
                </>
              ) : (
                <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-8">
                  <div className="flex items-center justify-center min-h-96">
