...
import type { ProductSummarySukosari } from '@/lib/queries/production-progress-protrack';

interface ProductSummaryCardSukosariProps {
  data: ProductSummarySukosari;
}
...
export default function ProductSummaryCardSukosari({ data }: ProductSummaryCardSukosariProps) {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      try {
        const res = await fetch(`/api/product/protrack-history?id_product=${encodeURIComponent(String(data.id_product))}&trainset=${encodeURIComponent(String(data.trainset))}&line=${encodeURIComponent(String(data.line ?? ''))}`);
    ...
    const fetchMissingSubProcess = async () => {
      ...
      try {
        //console.log('[ProductSummaryCardSukosari] Fetching missing subprocess for:', data.id_product, data.trainset);
        ...
        if (!res.ok) throw new Error('Fetch failed');
        const json = await res.json();
        //console.log('[ProductSummaryCardSukosari] Missing subprocess response:', json);
        if (!mounted) return;
        setMissingSubProcess(json.data || []);
        setMissingError(null);
      } catch (e) {
        if (!mounted) return;
        console.error('[ProductSummaryCardSukosari] Error fetching missing subprocess:', e);
        ...
        <div className="grid gap-2">
          ...
          {isExpanded && (
            <div className="border-t border-gray-700/50 pt-2">
              ...
              {((missingSubProcess && missingSubProcess.length > 0) || percentage < 100) && (
